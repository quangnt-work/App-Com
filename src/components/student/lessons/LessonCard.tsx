    ...
    <Card className="group overflow-hidden border-slate-200 bg-white hover:shadow-lg transition-all duration-300 rounded-2xl flex flex-col h-full">
      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="flex justify-between items-center mb-3">
          <span className={`text-[10px] font-bold px-2 py-1 rounded-sm uppercase tracking-wide ${getBadgeColor(lesson.category)}`}>
            {lesson.category}
          </span>
        ...
        <Link href={`/student/lessons/${lesson.id}`} className="block flex-1">
        ...
        <div className="flex items-center gap-3">
          <div className="relative w-8 h-8 rounded-full overflow-hidden border border-slate-100 bg-slate-100">
            {lesson.instructor_avatar ? (
                <Image src={lesson.instructor_avatar} alt="Ins" fill className="object-cover" />
            ) : (
